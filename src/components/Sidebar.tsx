...
const navItems = [
  ...
  { label: 'IA Audio', path: '/ia-audio', icon: 'M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z' },
]

const ADMIN_EMAIL = 't.coban@tayco.fr'

const bottomItems = [
  { label: 'Rapports', path: '/rapports', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
  { label: 'Conformite', path: '/conformite', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
  ...
  return (
    <motion.aside initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.3 }}
      ...
      <div className="px-6 h-16 flex items-center gap-3 border-b border-gray-100">
        <div className="w-9 h-9 bg-[#1E40AF] rounded-xl flex items-center justify-center flex-shrink-0">
          <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
          </svg>
        </div>
        <div><span className="text-[15px] font-bold text-gray-900 tracking-tight">TAYCOBAT</span>
      ...
      <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
        ...
        {navItems.map((item) => (
          <NavLink key={item.path} to={item.path} className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-xl text-[13px] font-medium transition-all ${isActive ? 'bg-[#1E40AF]/10 text-[#1E40AF]' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700'}`}>
        ...
        {email === ADMIN_EMAIL && (
          <NavLink to="/admin" className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-xl text-[13px] font-medium transition-all ${isActive ? 'bg-[#1E40AF]/10 text-[#1E40AF]' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700'}`}>
            <NavIcon d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4" />Admin
          </NavLink>
        )}
      </nav>
      ...
      <div className="px-3 pb-3 border-t border-gray-100 pt-3">
        <div className="flex items-center gap-3 px-3 py-2">
          {photoUrl ? (
            ...
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#1E40AF] to-blue-400 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0">{displayName.charAt(0).toUpperCase()}</div>
          )}
