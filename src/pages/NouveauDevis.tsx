...
export default function NouveauDevis() {
  ...
  useEmail() // available for future email sending
  ...
  const [dateValidite, setDateValidite] = useState(() => { const d = new Date(); d.setDate(d.getDate() + 30); return d.toISOString().split('T')[0] })
  ...
  // Mentions legales check
  const mentionsOk = !!(meta.siret && meta.adresse)

  // Generate numero
  useEffect(() => {
    if (!user) return
    const year = new Date().getFullYear()
    supabase.from('devis').select('id', { count: 'exact', head: true }).eq('user_id', user.id).like('numero', `DE-${year}-%`).then(({ count }) => {
      setNumero(`DE-${year}-${String((count ?? 0) + 1).padStart(4, '0')}`)
    })
  }, [user])
  ...
  async function handleCreateClient() {
    ...
    if (newType === 'particulier') { payload.nom = newNom; payload.prenom = newPrenom } else { payload.raison_sociale = newRaison }
  ...
  function setLigne(i: number, key: keyof Ligne, val: string | number) {
    setLignes((ls) => ls.map((l, j) => {
      if (j !== i) return l
      const u = { ...l, [key]: val }
      if (key === 'quantite' || key === 'prix_unitaire') u.total_ht = Math.round(Number(u.quantite) * Number(u.prix_unitaire) * 100) / 100
      return u
    }))
  }
  ...
  function moveLigne(from: number, to: number) { if (to < 0 || to >= lignes.length) return; setLignes((ls) => { const n = [...ls]; const [item] = n.splice(from, 1); n.splice(to, 0, item); return n }) }
  ...
  const tvaBreakdown = useMemo(() => {
    ...
    for (const l of prestations) { if (!map[l.tva_pct]) map[l.tva_pct] = { base: 0, tva: 0 }; map[l.tva_pct].base += l.total_ht; map[l.tva_pct].tva += Math.round(l.total_ht * l.tva_pct / 100 * 100) / 100 }
  ...
  const doSave = useCallback(async (finalize = false) => {
    ...
    setSaving(true)
    const payload = {
      numero, titre, client_id: clientId || null,
    ...
    else { const { data } = await supabase.from('devis').insert(payload).select('id').single(); if (data) { id = data.id; setDevisId(data.id) } }

    if (id) {
      // Save lignes to devis_lignes
      await supabase.from('devis_lignes').delete().eq('devis_id', id)
      const rows = lignes.filter((l) => l.type === 'prestation' && l.description).map((l, i) => ({
        devis_id: id, description: l.description, quantite: l.quantite, unite: l.unite,
        prix_unitaire: l.prix_unitaire, total_ht: l.total_ht, ordre: i, tva_pct: l.tva_pct,
      }))
      if (rows.length > 0) await supabase.from('devis_lignes').insert(rows)
      await logAudit({ user_id: user.id, action: devisId ? 'update' : 'create', table_name: 'devis', record_id: id, details: `Devis ${numero} ${finalize ? 'finalise' : 'sauvegarde'}` })
    }
    setLastSaved(new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }))
    setSaving(false)
    if (finalize) navigate('/devis')
    return id
  }, [user, numero, titre, clientId, netHT, totalTTC, tvaBreakdown, dateDevis, dateValidite, adresseChantier, devisId, lignes, navigate])
  ...
  async function generatePreviewPDF() {
    ...
    doc.setTextColor(30, 30, 30); doc.setFontSize(14); doc.setFont('helvetica', 'bold'); doc.text(entreprise, infoX, 17)
    ...
    doc.setDrawColor(200, 200, 200); doc.roundedRect(14, totY + 3, 76, 22, 2, 2); doc.roundedRect(120, totY + 3, 76, 22, 2, 2)
    if (hasAutoliquidation) { totY += 30; doc.setFontSize(7); doc.setTextColor(180, 120, 0); doc.text('Autoliquidation TVA — Art. 283-2 nonies du CGI. TVA due par le preneur assujetti.', 14, totY) }
    if (notesBasPage) { doc.setFontSize(7); doc.setTextColor(120, 120, 120); doc.text(notesBasPage, 14, 275, { maxWidth: 180 }) }
    doc.setFontSize(6); doc.setTextColor(150, 150, 150); doc.text('TAYCOBAT', 105, 288, { align: 'center' })
    return doc.output('datauristring')
  }

  const [pdfDataUri, setPdfDataUri] = useState<string | null>(null)
  useEffect(() => { if (tab === 'preview') generatePreviewPDF().then(setPdfDataUri) }, [tab, lignes, clientId, titre, adresseChantier, netHT, totalTTC])
    ...
    <div className="min-h-screen bg-[#f8f9fb] flex flex-col">
      ...
      <div className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-6 flex-shrink-0 sticky top-0 z-40">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/devis')} className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 cursor-pointer">
      ...
      {tab === 'preview' ? (
        <div className="flex-1 flex items-center justify-center p-8 bg-gray-100">
          {pdfDataUri ? <iframe src={pdfDataUri} className="w-full max-w-[800px] h-[85vh] rounded-xl border border-gray-200 shadow-lg bg-white" /> : <div className="text-gray-400 text-sm">Chargement...</div>}
        </div>
      ) : (
        <div className="flex-1 overflow-y-auto p-6">
          <div className="max-w-[1200px] mx-auto space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <input type="text" value={titre} onChange={(e) => setTitre(e.target.value)} placeholder="Titre du devis (ex: Renovation salle de bain, Ravalement facade...)" className={ic + ' text-lg font-semibold'} />
            </div>

            {/* EN-TETE 2 colonnes */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
                <div><label className="block text-xs font-semibold text-gray-400 uppercase mb-1">Numero devis</label>
                  <input type="text" value={numero} readOnly className={ic + ' bg-gray-50 text-gray-500 font-mono'} /></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-xs font-semibold text-gray-400 uppercase mb-1">Date du devis</label>
                    <input type="date" value={dateDevis} onChange={(e) => setDateDevis(e.target.value)} className={ic} /></div>
                  <div><label className="block text-xs font-semibold text-gray-400 uppercase mb-1">Valable jusqu'au</label>
                    <input type="date" value={dateValidite} onChange={(e) => setDateValidite(e.target.value)} className={ic} /></div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-xs font-semibold text-gray-400 uppercase mb-1">Debut des travaux</label>
                    <input type="date" value={debutTravaux} onChange={(e) => setDebutTravaux(e.target.value)} className={ic} /></div>
                  <div><label className="block text-xs font-semibold text-gray-400 uppercase mb-1">Duree estimee</label>
                    <input type="text" value={dureeEstimee} onChange={(e) => setDureeEstimee(e.target.value)} placeholder="Ex: 2 semaines" className={ic} /></div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
                ...
                {showNewClient && (
                  <div className="p-3 bg-gray-50 rounded-xl space-y-2 border border-gray-200">
                    ...
                    {newType === 'particulier' ? (
                      <div className="grid grid-cols-2 gap-2"><input value={newPrenom} onChange={(e) => setNewPrenom(e.target.value)} placeholder="Prenom" className={ic} /><input value={newNom} onChange={(e) => setNewNom(e.target.value)} placeholder="Nom *" className={ic} /></div>
                    ) : (
                      <div className="flex gap-2">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-gray-50">
                {lignes.length === 0 && (
                  <div className="py-12 text-center text-sm text-gray-400">Cliquez sur un des boutons ci-dessous pour ajouter un element</div>
                )}
                {lignes.map((l, i) => {
                  ...
                  return (
                    <div key={i} className="grid grid-cols-[40px_3fr_80px_80px_110px_90px_110px_40px] gap-1 px-4 py-2 items-start hover:bg-gray-50/50">
                      ...
                      <div className="flex flex-col gap-0.5 pt-1">
                        ...
                        <button onClick={() => duplicateLigne(i)} className="p-0.5 text-gray-300 hover:text-blue-500 cursor-pointer"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg></button>
                        <button onClick={() => removeLigne(i)} className="p-0.5 text-gray-300 hover:text-red-500 cursor-pointer"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                      </div>
                    </div>
                  )
                })}
            ...
            {/* NOTES BAS DE PAGE */}
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <label className="block text-xs font-semibold text-gray-400 uppercase mb-1">Notes de bas de page</label>
              <textarea value={notesBasPage} onChange={(e) => setNotesBasPage(e.target.value)} rows={3} placeholder="Mentions legales, conditions generales, garanties..." className={ic + ' resize-vertical min-h-[60px]'} />
            </div>
