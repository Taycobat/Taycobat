...
export default function NouvelleFacture() {
  ...
  async function generatePreviewPDF() {
    ...
    doc.setDrawColor(...green); doc.setLineWidth(0.8); doc.line(14, 34, 196, 34)
    ...
    autoTable(doc, {
      ...
      bodyStyles: { fontSize: 8 }, alternateRowStyles: { fillColor: [245, 250, 247] },
          ...
          <div className="flex gap-1 bg-gray-100 rounded-lg p-0.5">
            {[{ k: 'edition' as const, l: 'Edition' }, { k: 'preview' as const, l: 'Previsualisation' }].map((t) => (
              <button key={t.k} onClick={() => setTab(t.k)}
                className={`px-3 py-1.5 rounded-md text-xs font-semibold cursor-pointer transition-all ${tab === t.k ? 'bg-white text-[#1a9e52] shadow-sm' : 'text-gray-500'}`}>{t.l}</button>
            ))}
          </div>
          ...
          <div className="max-w-[1200px] mx-auto space-y-6">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={() => setShowNewClient(!showNewClient)}
                      className={`w-10 h-10 rounded-xl border flex items-center justify-center cursor-pointer flex-shrink-0 ${showNewClient ? 'border-[#1a9e52] bg-emerald-50 text-[#1a9e52]' : 'border-gray-200 text-gray-400 hover:text-[#1a9e52] hover:border-[#1a9e52]'}`}>
                  ...
                  <div className="p-3 bg-gray-50 rounded-xl space-y-2 border border-gray-200">
                    ...
                    {newType === 'particulier' ? (
                      ...
                      <div className="flex gap-2">
                        ...
                        }} className="px-3 py-2 text-xs font-semibold text-white bg-[#1a9e52] rounded-lg cursor-pointer disabled:opacity-40">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                {lignes.map((l, i) => {
                  if (l.type === 'section') return (
                    <div key={i} className="flex items-center gap-2 px-4 py-2 bg-emerald-50 border-l-4 border-[#1a9e52]">
                      <span className="text-xs font-bold text-[#1a9e52] uppercase w-10">{i + 1}</span>
                    ...
                    <div key={i} className="grid grid-cols-[40px_3fr_80px_80px_110px_90px_110px_40px] gap-1 px-4 py-2 items-start hover:bg-gray-50/50">
                      ...
                      <div className="flex gap-1">
                        <textarea value={l.description} onChange={(e) => { setLigne(i, 'description', e.target.value); e.target.style.height = 'auto'; e.target.style.height = Math.min(160, Math.max(72, e.target.scrollHeight)) + 'px' }}
                          ...
                          className="flex-1 px-2 py-2 rounded-lg border border-gray-200 bg-white text-[14px] leading-5 focus:outline-none focus:ring-2 focus:ring-[#1a9e52]/20 focus:border-[#1a9e52] resize-vertical min-h-[72px] max-h-[160px]" />
              ...
              <div className="flex items-center justify-between px-4 py-3 bg-gray-50 border-t border-gray-100">
                <div className="flex gap-2">
                  <button onClick={() => addLigne('prestation')} className="px-3 py-1.5 text-xs font-semibold text-[#1a9e52] bg-emerald-50 hover:bg-emerald-100 rounded-lg cursor-pointer">+ Prestation</button>
            ...
            <button onClick={() => { if (ajustement === 0) setAjustement(0.01) }} className="text-xs text-[#1a9e52] font-semibold hover:underline cursor-pointer">+ Definir un ajustement (remise)</button>
